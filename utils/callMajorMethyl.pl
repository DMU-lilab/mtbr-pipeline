...
sub main {
    my (@F, $ref, $pos, $strand, $base, $cross, $crossred, $call, $cov, $rate, $countT, $countC);
	...
	## get methlation call
	($call, $countC, $countT, $cov, $rate) = getMethylCall($crossred, $STRAND);

	## print output
	#print $OUT join("\t", ($ref, $pos, $call, $strand, $cov, $rate))."\n" if $call =~ /[CT]/;
	print $OUT join("\t", ($ref, $pos, $strand, $countC, $countT))."\n" if $call =~ /[CT]/;
    }
...
sub getMethylCall {
    my ($cross) = @_;
    my (%occ, $ch, $cnt, $call, $cov, $rate, $countT, $countC);
...
#    return ($call, $cov, $rate);
	return($call, $countC, $countT, $cov, $rate);
}
...
sub getReducedCross {
    ...
    ## exclude indels
    ## e.g. "[+-]2AT" ==> ""
    $cross =~ s/[+-](\d+)([a-zA-Z]+)/substr($2,$1)/eg;
